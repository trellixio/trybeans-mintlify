...
Merchants can also define tier-specific benefits to enhance the attractiveness of each tier.
These benefits are illustrative examples only. Beans does not implement them automatically
or provide built-in functionality for these features. Instead, the tier system offers a
flexible framework for you to design perks that align with your brand, which you will need
to configure and manage independently.

For example, you might create benefits such as:
...
<Info>
  {' '}
  The examples above are not default features of Beans. They represent possibilities that require manual setup,
  integration with your ecommerce platform to activate. Beans enables the structure for tier customization, but
  implementation depends on your specific configuration.
</Info>
...
3. Define the number of tiers you want to create (up to 3 tiers).
4. Set the points multiplier for each tier.
5. Define the specific benefits for each tier.
6. Save your settings to activate the tiers.
