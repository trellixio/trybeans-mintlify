...
In addition, the redemption settings allow you to set how customers can use their points.
...
And the default rate of 100 points to 1 unit of currency makes it possible for the customer
to have a large digit amount of points, for example, millions or billions of points.
While this is not a technical limitation, it can disturb the customer experience and cause
display overflow in some places. The Beans team is aware of this issue and is working on solutions.
...
#### What happens to points when a purchase fails or is abandoned?

When customers redeem points for a purchase, Beans creates a pending debit transaction.
If the purchase is not completed (due to payment failure, cart abandonment, etc.) within 48 hours,
the points are returned automatically to the customer's account and can be used for future purchases.
This ensures customers never lose points due to incomplete transactions.
