...
When customers redeem points for a purchase, Beans creates a pending debit transaction. If the purchase is not completed
(due to payment failure, cart abandonment, etc.) within 48 hours, the points are returned automatically to the customer's
account and can be used for future purchases. This ensures customers never lose points due to incomplete transactions.
